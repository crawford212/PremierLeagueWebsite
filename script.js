...
async function openModal(team) {
  ...
  try {
    ...
    // ── DEBUG: log every PL 2024 stat for every player returned by this team query
    console.log(`\n=== ALL PL 2024 STATS FOR ${team.team.name} (id:${team.team.id}) ===`);
    uniquePlayers.forEach(p => {
      (p.statistics || []).forEach(s => {
        if (Number(s.league?.id) === 39 && Number(s.league?.season) === 2024) {
          console.log(
            `  ${p.player.name.padEnd(25)} | stat_team_id: ${String(s.team?.id).padEnd(5)} | stat_team_name: ${String(s.team?.name).padEnd(20)} | goals: ${s.goals?.total ?? 'null'} | assists: ${s.goals?.assists ?? 'null'} | mins: ${s.games?.minutes ?? 'null'}`
          );
        }
      });
    });
    console.log(`=== END (queried team id: ${team.team.id}) ===\n`);

    const agg = {};
    uniquePlayers.forEach(p => {
      (p.statistics || []).forEach(s => {
        if (Number(s.league?.id)     !== 39)           return;
        if (Number(s.league?.season) !== 2024)         return;
